...
-- Save most suitable promotion code reward against the basket
ALTER TABLE `baskets` ADD COLUMN `promotion_code_reward_id` int AFTER `promotion_code_id`, CHANGE COLUMN `created` `created` datetime NOT NULL AFTER `promotion_code_reward_id`, CHANGE COLUMN `modified` `modified` datetime DEFAULT NULL AFTER `created`, CHANGE COLUMN `deleted` `deleted` datetime DEFAULT NULL AFTER `modified`;